...
function WallDebug({ city }: { city: CityModel }) {
  ...
  return(
    <>
      ...
      {city.gates.map((gate) => {
        let end = city.plaza
        ? city.plaza.shape.minPredicate(v => v.distanceTo(gate))
        : city.center

      let street = city.topology.buildPath(
        gate,
        end,
        [] //Array.from(city.topology.outer.values()),
      )

      console.log({street})
        return <>
        <circle cx={gate.x} cy={gate.y} r="5" fill="red" />
        <circle cx={end.x} cy={end.y} r="5" fill="purple" />
        
        </>
      })}
       {Array.from(city.topology.nodeToVector3.values())?.map(({ x, y }) => {
        return <circle cx={x} cy={y} r="1" fill="orange" />
      })}
      ...
      {city.streets.map(({ vertices }, i) => {
        if (vertices.length < 2) {
          if (vertices.length === 1) {
            return <circle  cx={vertices[0].x} cy={vertices[0].y} r="5" fill="pink" />
          }
          return null
        }
        const points = vertices.map(({ x, y }) => `${x},${y}`).join(" ")
    ...
    >
      ...
      {CityDebug}
        ...
        {city && <header style={{ position: "fixed", top:0}}>
          ...
          <h1>
            City Debug Test Thing: press enter to re-roll city
          </h1>
          <p>

        Options: {city.wallsNeeded && "walls"} | {city.citadelNeeded && "citadel"} | {city.plazaNeeded && "plaza"}
        </p>
        </header>}
        {/* </UncontrolledReactSVGPanZoom> */}
