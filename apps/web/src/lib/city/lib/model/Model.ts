...
export class CityModel {
	...
	private buildWalls() {
		...
		const reservedShape = this.citadel != null ? this.citadel.shape.copy() : new Polygon([]);

		this.border = new CityWall(this.wallsNeeded, this, this.inner, reservedShape.vertices);
		...
		this.patches = this.patches.filter(p => p.shape.minDistance(this.center) < radius * 3);
		...
		if (this.citadel) {
			...
			if (this.citadel.shape.compactness < 0.76) throw new Error('Bad citadel shape!');
	...
	private buildStreets() {
		...
		for (let gate of this.gates) {
			...
			let end = this.plaza ? this.plaza.shape.minPredicate(v => v.distanceTo(gate)) : this.center;
			...
			console.log('Build Street', { street, gate, end });
			...
			if (street) {
				...
				if (this.border.gates.includes(gate)) {
					...
					let road = this.topology.buildPath(start, gate, Array.from(this.topology.inner.values()));
					if (road) {
	...
	private tidyUpRoads() {
		...
		const cut2segments = (street: Street) => {
			...
			for (let i = 1; i < street.length; i++) {
				...
				if (this.plaza && this.plaza.shape.contains(v0) && this.plaza.shape.contains(v1)) continue;
	...
	private createWards() {
		...
		for (let gate of this.border.gates) {
			for (let patch of this.patchByVertex(gate)) {
				if (patch.withinCity && !patch.ward && this.random.bool(!this.wall ? 0.2 : 0.5)) {
	...
	public getNeighbors(patch: Patch): Array<Patch> {
		return this.patches.filter(p => p !== patch && p.shape.bordersPolygon(patch.shape));
	}
	...
	public isEnclosed(patch: Patch) {
		return (
			patch.withinCity && (patch.withinWalls || this.getNeighbors(patch).every(p => p.withinCity))
		);
	}
}
