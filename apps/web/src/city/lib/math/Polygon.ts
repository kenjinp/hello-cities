...
function lineIntersection(line1: LineSegment, line2: LineSegment): Vector3 | null {
...
function isPointOnLineSegment(point: Vector3, segmentStart: Vector3, segmentEnd: Vector3): boolean {
	...
	return (
		Math.abs(distanceStartToPoint + distanceEndToPoint - segmentLength) < epsilon &&
		distanceStartToPoint < segmentLength + epsilon &&
		distanceEndToPoint < segmentLength + epsilon
	);
}

function splitPolygon(polygon: Polygon, lineSegment: LineSegment, gap): Polygon[] {
	...
	let half1 = new Polygon(polygon.vertices.slice(point1.edgeIndex + 1, point2.edgeIndex + 1));
	half1.vertices.unshift(point1.point);
	half1.vertices.push(point2.point);

	let half2 = new Polygon(
		polygon.vertices.slice(point2.edgeIndex).concat(polygon.vertices.slice(0, point1.edgeIndex + 1))
	);
...
export const scalar = (x1: number, y1: number, z1: number, x2: number, y2: number, z2: number) =>
	x1 * x2 + y1 * y2 + z1 * z2;
...
export class Polygon {
	...
	public forEdge(callback: (a: Vector3, b: Vector3, edgeIndex: number) => void | boolean) {
		for (let i = 0; i < this.vertices.length; i++) {
			if (callback(this.vertices[i], this.vertices[(i + 1) % this.vertices.length], i) === false) {
				break;
			}
		}
	}
	...
	public prev(v: Vector3) {
		return this.vertices[
			(this.vertices.indexOf(v) + this.vertices.length - 1) % this.vertices.length
		];
	}
	...
	public smoothVertex(v: Vector3, f = 1.0): Vector3 {
		...
		return v
			.clone()
			.set(prev.x + v.x * f + next.x, prev.y + v.y * f + next.y, prev.z + v.z * f + next.z)
	...
	public split(p1: Vector3, p2: Vector3) {
		return this.splitAtIndex(this.vertices.indexOf(p1), this.vertices.indexOf(p2));
	}
	...
	public minDistance(v: Vector3) {
		return this.vertices.reduce((min, p) => Math.min(min, p.distanceTo(v)), Infinity);
	}
	...
	public triangulate(): Vector3[] {
		function isEar(p1: Vector3, p2: Vector3, p3: Vector3, points: Vector3[]): boolean {
			// Check if the angle at p2 is concave.
			if (crossProductLength(p1, p2, p3) >= 0) {
				return false;
			}
		...
		function crossProductLength(p1: Vector3, p2: Vector3, p3: Vector3): number {
			const v1x = p2.x - p1.x;
			const v1y = p2.y - p1.y;
			const v2x = p3.x - p2.x;
			const v2y = p3.y - p2.y;
		...
		function isPointInTriangle(p: Vector3, p1: Vector3, p2: Vector3, p3: Vector3): boolean {
			const b1 = crossProductLength(p, p1, p2) < 0;
			const b2 = crossProductLength(p, p2, p3) < 0;
			const b3 = crossProductLength(p, p3, p1) < 0;
			return b1 === b2 && b2 === b3;
		}

		const triangles: Vector3[] = [];
		let remainingPoints = [...this.vertices].slice();

		while (remainingPoints.length >= 3) {
			const n = remainingPoints.length;
			let earIndex = -1;

			// Check each vertex to see if it forms an "ear".
			for (let i = 0; i < n; i++) {
				const prev = remainingPoints[(i + n - 1) % n];
				const curr = remainingPoints[i];
				const next = remainingPoints[(i + 1) % n];
			...
			if (earIndex === -1) {
				// The polygon is not convex, or it has holes.
				// We cannot triangulate it using this algorithm.
				throw new Error('Polygon is not convex.');
			}

			// Create a triangle using the ear vertex and its adjacent vertices.
			const prev = remainingPoints[(earIndex + n - 1) % n];
			const curr = remainingPoints[earIndex];
			const next = remainingPoints[(earIndex + 1) % n];
			triangles.push(prev, curr, next);

			// Remove the ear vertex from the list of remaining points.
	...
	public vectorByIndex(i: number) {
		return this.vertices[i == this.vertices.length - 1 ? 0 : i + 1].clone().sub(this.vertices[i]);
	}
	...
	public insetPolygonEdgeByVertex(v1: Vector3, gapSize: number): Polygon {
		...
		const prevIndex = startVertexIndex === 0 ? vertices.length - 1 : startVertexIndex - 1;
		const nextIndex = startVertexIndex === vertices.length - 1 ? 0 : startVertexIndex + 1;
		...
		const prevPerpLength = Math.sqrt(
			prevPerpendicular.x * prevPerpendicular.x + prevPerpendicular.y * prevPerpendicular.y
		);
		const nextPerpLength = Math.sqrt(
			nextPerpendicular.x * nextPerpendicular.x + nextPerpendicular.y * nextPerpendicular.y
		);
		const prevUnit = {
	...
	public insetAllEdgesByDistances(d: Array<number>): Polygon {
		...
		while (regular.length > 0) {
			...
			let p: Polygon = new Polygon(new Array(indices.length).fill(0).map((_, i) => q.vertices[i]));
			let s = p.square;
			if (s > bestPartSq) {
				bestPart = p;
				bestPartSq = s;
